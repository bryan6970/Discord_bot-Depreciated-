...
necessary_files = {
    "hyperparams": {"data": dict, "message": discord.Message, "filename": "hyperparams.json"},
    "openai_api": {"data": dict, "message": discord.Message, "filename": "openai_api.txt"},
    "function_permissions": {"data": dict, "message": discord.Message, "filename": "function_permissions.json"},
    "member_details": {"data": dict, "message": discord.Message, "filename": "member_details.json"},
}
...
async def on_ready():
    ...
    try:
        with open("member_details.json", "r") as f:
            client.existing_members = json.load(f)
    except FileNotFoundError:
        client.existing_members = {}

    try:
        with open("../../testing/hyperparams.json", "r") as f:
            client.hyperparams = json.load(f)
    except FileNotFoundError:
        with open("../../testing/hyperparams.json", "w") as f:

            client.hyperparams = {"bot responses fun": True, "reply responses": True}
            json.dump(client.hyperparams, f)

    try:
        client.bot_responses_fun = client.hyperparams["bot responses fun"]
...
async def on_command_error(ctx, error):
    ...
    elif isinstance(error, commands.CheckFailure):  # this is the final elif
...
async def commands(ctx):
    ...
    for func, details in function_details.items():
        ...
        # Format the definition
        definition = "".join([f"{sentence}.\n         |" for sentence in details["Definition"].split(".")])
...
async def SETX(ctx, app_name, *params):
    if app_name.upper() == "VALTRACKER":
        ...
        await save_json_files(necessary_files["member_details"]["message"], "member_details.json", member_details)
...
async def OFF(ctx, feature: str):
    ...
    attachments.append(discord.File(json_file, filename="../../testing/hyperparams.json"))

    await message.delete()
    await bot_data_channel.send(files=attachments) @ has_permission
...
async def on_message(message):
    global ready, bot_data_channel, hyperparams, function_permissions, necessary_files
    print("735")
    ...
    if not ready:
        ...
        if channel is not None:
            ...
            for message in bot_data:
                for attachment in message.attachments:
                    if attachment.filename == "hyperparams.json":
                        necessary_files["hyperparams"] = attachment.to_dict()
                        necessary_files["hyperparams"]["hyperparams_message"] = message
                    elif attachment.filename == "openai_api.txt":
                    ...
                    elif attachment.filename == "function_permissions.json":
                        with open("function_permissions.json", "r") as f:
                            default_file = json.load(f)
                            dict_from_discord = attachment.to_dict()
                            if default_file.keys() == dict_from_discord.keys():
                                necessary_files["function_permissions_message"] = dict_from_discord
                            else:
                        ...
                        necessary_files["function_permissions_message"][
                            "function_permissions_message_message"] = message
                    elif attachment.filename == "member_details.json":
                        necessary_files["member_details_message"] = attachment.to_dict()
                        necessary_files["member_details_message"]["member_details_message_message"] = message
    ...
    if message.channel.id == test_channel_id :
        ...
        if outputs[0]["label"] != "OK" and outputs[0]["score"] > 0.5:
            ...
            print(f"OK score {round(outputs[0]['score'], 2)}")
...
async def member_ids(guild):
    ...
    async for member in guild.fetch_members():
        if str(member.id) in member_details:
            # Update the name if the member ID exists
            member_details[str(member.id)]["name"] = member.global_name if member.nick is None else member.nick
